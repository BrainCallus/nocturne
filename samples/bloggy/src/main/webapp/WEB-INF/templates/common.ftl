...
<#macro page>
    ...
    <header>
        ...
        <div class="enter-or-register-box">
            ...
            <#else>
                <a href="<@link name="EnterPage"/>">{{Enter}}</a> {{or}} <a href="<@link name="RegisterPage"/>">{{Register}}</a>
            </#if>
        </div>
        <nav>
            <ul>
...
<#macro shortText maxLen short entity>
    <div class="_text" style="word-break: break-word;">
        <#if short && (entity.text?length>maxLen)>
            <div class="_preview${entity.id}_">
                ${entity.text?substring(0, maxLen)?html}... <a href="#${entity.id}_" class="_full">[{{view all}}
                    &rarr;]</a>
            </div>
            <div class="_complete${entity.id}_" style="display: none">
                ${entity.text?html}
            </div>
        <#else>
            ${entity.text?html}
    ...
    <script>
        $("._full").css("cursor", "pointer").click(function () {
            var $text = $(this).closest("div._text");
            $text.find("._preview${entity.id}_").hide();
            $text.find("._complete${entity.id}_").show();
        });
    </script>
</#macro>
